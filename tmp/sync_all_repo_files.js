...
async function makeRequest(token, endpoint, payload) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({
      hostname: "localhost",
      port: 8001, // hit the real API directly
      path: endpoint,
      method: "POST",
      headers: {
...
async function main() {
  try {
    ...
    if (!fs.existsSync(srcDir)) {
      console.error(`Source directory ${srcDir} does not exist. Make sure you cloned it.`);
      return;
    }
    ...
    // 2. Write files in batches of 10 to keep payloads reasonable
    const batchSize = 10;
    ...
    console.log("SUCCESS: All files synchronized perfectly through the control plane!");
  } catch (err) {
